...
# Siste kjente posisjon per node (inkl altitude)
node_positions = {}   # {node_id: {"lat":..., "lon":..., "alt":..., "time":...}}

# Siste kjente telemetry per node (for strøm/evt ping)
...
def append_csv_row(row: dict):
    # Kall alltid innenfor lock hvis du vil være helt trygg
...
def on_receive(packet, interface):
    ...
    # --- Prøv å hente "channel" best-effort ---
    channel = (
        packet.get("channel")
    ...
    if port == "POSITION_APP":
        ...
        # Altitude kan hete litt forskjellig – prøver flere
    ...
    # Telemetry (for “strømforbruk” / evt ping hvis det finnes)
    if port == "TELEMETRY_APP":
        tel = decoded.get("telemetry", {}) or {}

        # Meshtastic kan gi deviceMetrics / environmentMetrics
        device = tel.get("deviceMetrics", {}) or {}
        env = tel.get("environmentMetrics", {}) or {}

        # Strømforbruk finnes ofte ikke som “current”, men vi prøver:
        strom = (
            device.get("current") or device.get("currentMa") or device.get("current_mA") or
            device.get("power") or device.get("powerMw") or device.get("power_mW") or
            device.get("batteryCurrent") or device.get("batteryCurrentMa")
        )

        # Ping/RTT kan i noen varianter finnes – vi prøver:
        ping_ms = device.get("rttMs") or device.get("pingMs") or tel.get("rttMs")

        node_telemetry[from_id] = {
            "time": time.time(),
            "stromforbruk": strom,
            "ping_ms": ping_ms,
        }
    ...
    # Siste pos/telemetry (om den finnes)
    pos = node_positions.get(from_id) or {}
    tel = node_telemetry.get(from_id) or {}
    ...
    # --- CSV: skriv én rad per tekstmelding ---
    # ping_ms / strømforbruk blir tomt hvis ikke vi har sett telemetry med slikt
    # altitude blir tomt hvis pos ikke inneholder altitude
    row = {
        ...
        "ping_ms": tel.get("ping_ms"),
        "stromforbruk": tel.get("stromforbruk"),
        "channel": channel,
    }
