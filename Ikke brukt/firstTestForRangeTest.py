...
def on_receive(packet, interface):
    ...
    if portnum == "TEXT_MESSAGE_APP":
        ...
        # CORRECTED ECHO LOGIC
        def send_echo():
            time.sleep(2)
            print(f" Echoing back to {long_name}...")
            try:
                # 'destinationId' is the correct keyword for the Python API
                interface.sendText(f"ACK: {msg_text}", destinationId=from_id)
            except Exception as e:
                print(f" Failed to echo: {e}")
        
        threading.Thread(target=send_echo).start()
...
def main():
    print("--- Meshtastic Echo & Range Logger Started ---")
